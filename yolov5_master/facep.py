...
def get_featuresdict(model, dir):
    ...
    for i,each in enumerate(list):
        image = load_image(f"D:\Pycharm\pycharmProject\yolov5_web\static\pic\{each}")
...
def detect(save_img=False,view_img=False):
    ...
    dir = "D:\Pycharm\pycharmProject\yolov5_web\static\pic"
            ...
            if det is not None and len(det): # 假如预测到有目标（盒子存在）
                ...
                for *xyxy, conf, cls in det: # x1 y1 x2 y2 cls class
                    ...
                    # pred real or fack
                        ...
                        if rf_label==1 and value>0.90: label+=""
                        else:label+=""
                    ...
                    #         if count >= 10:
                    #             count = 0
                    #             if time.time() - tplay > 15.0:
                    #                 os.system(
                    #                     'start /b D:\Pycharm\pycharmProject\yolov5_web\\voice\\bin\\ffplay.exe -autoexit -nodisp D:\Pycharm\pycharmProject\yolov5_web\\voice\\notFind.mp3')  # 音乐播放
                    ...
                    #         if count1 >= 5:
                    #             count1 = 0
                    #             if time.time() - showTime > 10.0:
                    #                 # 打开文件以追加模式写入标签
                    #                 file_path = 'D:\\Pycharm\\pycharmProject\\yolov5_web\\information\\' + Time + '.txt'
                    ...
                    #
                    #                 os.system(
                    #                     'start /b D:\Pycharm\pycharmProject\yolov5_web\\voice\\bin\\ffplay.exe -autoexit -nodisp D:\Pycharm\pycharmProject\yolov5_web\\voice\success.mp3')  # 音乐播放
                    #                 # 参数含义： start /b 后台启动  音乐播放软件的位置   -autoexit 播放完毕自动退出  -nodisp不显示窗口    mp3语音的位置
                    #                 showTime = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yoloV5_weights', type=str, default='D:\Pycharm\pycharmProject\yolov5_web\yolov5_master\weights\yolov5_weights\\best.pt', help='yolov5.pt path')
    parser.add_argument('--Areface_weights', type=str, default='D:\Pycharm\pycharmProject\yolov5_web\yolov5_master\weights\\arcface_weights\\resnet18_110.pth', help='arcface.pth path')
    parser.add_argument('--silentFace_weights', type=str, default='D:\Pycharm\pycharmProject\yolov5_web\yolov5_master\weights\\anti_spoof_models', help='manti_spoof_models dir path')
    parser.add_argument('--source', type=str, default='D:\Pycharm\pycharmProject\yolov5_web\static\downloads\\uploaded_image.jpg', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--output', type=str, default='D:\Pycharm\pycharmProject\yolov5_web\static\exp', help='output folder')  # output folder
